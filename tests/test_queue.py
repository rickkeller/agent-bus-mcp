...
def test_existing_state_directory_is_private(tmp_path: Path) -> None:
    ...
    DurableQueue(tmp_path, producer_id="producer", worker_id="worker")
...
def test_configured_route_binds_principals_to_api_objects(tmp_path: Path) -> None:
    policy = RoutePolicy(
        agents={"scheduled", "consumer", "other"},
        routes={("scheduled", "consumer"): {"task"}},
    )
    producer = DurableQueue(tmp_path, policy=policy, agent_id="scheduled")
    consumer = DurableQueue(tmp_path, policy=policy, agent_id="consumer")
    other = DurableQueue(tmp_path, policy=policy, agent_id="other")
    task = producer.enqueue(goal="read", references=[], idempotency_key="route", destination_id="consumer")
    assert other.claim() is None
    claim = consumer.claim()
    assert claim and claim["task_id"] == task
    with pytest.raises(QueueRefused):
        other.finish(task_id=task, lease_id=claim["lease_id"], result="done", failed=False)
