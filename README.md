...
Agent Bus MCP lets one software agent ask another for quick advice or hand off a clearly defined job.

## Two ways to use it

- **Quick question:** ask for one short piece of advice, such as “Which heading is clearer?” The question expires, one configured adviser may answer it once, and nothing is carried out automatically.
- **Formal task:** hand off a bounded job with a goal and optional public web references. The configured worker claims it, then marks it completed or failed.
...
A quick question has no structured fields for file or web references, payloads, execution scope, attachments, a destination or return route, task completion, or delivery authority. Do not put commands or credentials in its free text. An answer cannot create or finish a task. Remote clients cannot create questions or tasks through this server; creation stays on the configured local producer side.

## How the boundary works

Agent Bus MCP is a small, vendor-neutral, MIT-licensed Python service for one configured producer identity and one configured worker identity. It keeps task and consultation records in separate directories under a private state root. A single process lock, atomic JSON replacement, file and directory `fsync`, idempotency records, expiring leases, and lease fencing make state transitions durable and predictable.
...
Identities use a closed format and are fixed at startup. MCP calls cannot choose or override them. No tool fetches URLs, opens files, runs a shell command, routes to another worker, or creates a consultation or task.
...
from agent_bus_mcp import DurableQueue
...
answer = queue.read_answer(
    question["consultation_id"],
    producer_id="example_producer",
)
```

A consultation binds its generated ID, configured producer and worker IDs, caller-provided origin reference, idempotency key, question, creation/claim/answer timestamps, expiry, status, lease, and terminal answer. Retrying `ask_question` with the same idempotency key returns the original record. Questions and answers are each limited to 2048 UTF-8 bytes; origin references are limited to 256 bytes; expiry must be 1–86400 seconds. Text containing disallowed control characters is refused.
...
Existing task behavior is unchanged. A local producer calls `enqueue` with a bounded goal, zero to four syntactically public-HTTPS references, and an idempotency key. The MCP worker calls `claim_task`, then either `complete_task` with a result of at most 2048 bytes or `fail_task` without a result.
...
This is intentionally not full Streamable HTTP: there are no sessions, SSE streams, resumability, server-initiated messages, heartbeats, or multi-worker routing. Confirm that the selected MCP client can use this stateless POST-only contract.
...
The state root is forced to mode 0700. The bearer secret must be unique, high entropy, private, and rotated after suspected exposure. The bind setting must be `loopback`; the port must be decimal. Both IDs must be 1–64 characters from `[A-Za-z0-9_-]` and must match between producer and bridge.
...
The checked-in public-research manifest is an example formal-task producer, not the product. Its reference uses the reserved `.invalid` domain and is never fetched by this package. The included user-service examples load `%h/.config/agent-bus-mcp/env` and preserve state at `%h/.local/state/agent-bus-mcp`.
...
It is not a general job platform, a remote creation API, a network-fetching agent, a full Streamable HTTP server, a result-trust boundary, or a substitute for an operator’s TLS, authorization, backup, and incident-response decisions. See [SECURITY.md](SECURITY.md).
